...
#' @param reduction (character) reduction to visualize
#' @param components (numeric vector) which components to visualize
#' @param by (character) groupings for cells
#' @param type (character) type of groupings (one of 'discrete' or 'continuous')
#' @param pt.size (numeric) size of points
#' @param pt.alpha (numeric) transparency of points
#' @param col.midpoint (numeric) for continuous labels - midpoint of color gradient
#' @param col.low (character) for continuous labels - color gradient low
#' @param col.mid (character) for continuous labels - color gradient mid
#' @param col.high (character) for continuous labels - color gradient high
#' @param col.discrete (character) for discrete labels - colors for groupings
#' @param main (character) title
#' @param main.size (numeric) size of title
#' @param legend (logical) show legend
#' @param legend.text.size (numeric) size of legend text
#' @param legend.pt.size (numeric) size of legend point
#'
...
plotReduction <-
    function(bank,
        ...
        if (is.null(by))
            plot <-
            ggplot(data, aes(x = !!ensym(x), y = !!ensym(y)))
        else {
        ...
        plot <-
            plot + geom_point(size = pt.size, alpha = pt.alpha) + ggtitle(main) +
            theme_blank(
...
plotSpatial <- function(bank,
    ...
    if (is.na(by))
        ...
        if (type == 'continuous') {
            if (is.null(col.midpoint))
                col.midpoint <- median(feature)
            if ((is.null(col.highpoint)) |
                (is.null(col.lowpoint))) {
                plot <- ggplot(data, aes(
                    x = sdimx,
                    y = sdimy,
                    col = feature
...
#'
plotSpatialFeatures <- function(bank,
                                by,
                                type,
                                dataset = NULL,
                                nrow = NULL,
    ...
    if (is.null(col.discrete)) {
        plots <- Map(
            f = function(feature, feature.type, ...) {
                plotSpatial(
                    bank,
                    dataset = dataset,
                    by = feature,
...
#' @param col.breaks color breaks to use in heatmap (same number as col is specified)
...
#' @param cluster_row_slices cluster row slices (BANKSY)
#' @param show_row_dend show row dendrograms
#' @param show_row_names show row names
#' @param cluster_rows cluster rows
#' @param cex.row row label size
#' @param row_gap row gaps between row slices
#' @param cluster_column_slices cluster column slices (annotations)
#' @param show_column_dend show column dendrograms
    ...
    function(bank,
        ...
        if (annotate) {
            ...
            anno <- getCellAnnotation(
                ...
                order.by = order.by,
                col.discrete = col.discrete
            )
            top_anno <- anno$anno
            col_split <- anno$cell.split
            cell_order <- anno$cell.order
        } else {
            top_anno <- NULL
            col_split <- NULL
            cell_order <- colnames(mat)
        }
...
plotARI <-
    function(bank,
        ...
        data$Var2 <-
            factor(data$Var2, levels = rev(colnames(ari.mat)))
        ...
        if (label)
            tile <-
            tile + geom_text(aes(label = round(value, digits)))
        return(tile)
    }
            ...
            if (is.list(bank@own.expr)) {
                ...
                mat <-
                    Map(function(lam, mat)
                        mat * lam,
                        mat,
                        getLambdas(lambda, length(mat) - 1))
...
getCellAnnotation <- function(bank,
                              mat,
                              annotate.by,
                              annotation.name,
                              annotation.size,
    ...
    if (is.null(annotate.by) |
        !all(annotate.by %in% names(bank@meta.data))) {
    ...
    if (is.null(order.by)) {
        ...
        if (length(annotate.by) > 1) {
            message('Metadata to order cells by not specified. Using ',
                    order.by)
        }
    ...
    hc <-
        lapply(mdata, FUN = getDiscretePalette, col.discrete = col.discrete)
...
getReduction <- function(bank, reduction, components) {
    if (is.null(reduction)) {
        reduction <- names(bank@reduction)[length(bank@reduction)]
    }
    ...
    if (!valid)
        stop('Only 2 components may be visualized, not ',
             length(components))
    data <- NULL
    if (grepl('pca', reduction))
        data <- bank@reduction[[reduction]]$x[, components]
    if (grepl('umap', reduction))
        data <- bank@reduction[[reduction]][, components]
    if (is.null(data))
        stop('Specify a valid reduction')
...
getDiscretePalette <- function(feature, col.discrete = NULL) {
    ...
    if (is.null(col.discrete)) {
        ...
        if (length(col.discrete) < n)
            stop('Not enough colors')
        pal <- col.discrete
        if (is.null(names(col.discrete)))
            stop('
       Color palatte must be supplied with valid cluster names.')
        if (!all(unique(feature) %in% names(col.discrete)))
            stop(
...
sampleMatrix <- function(mat, max.cols, seed) {
    ...
    keep <-
        sample(colnames(mat), min(length(colnames(mat)), max.cols))
