...
lam <- c(0, 0.25) # lambda
res <- c(1.5) # leiden clustering resolution
...
```{r, echo=TRUE}
# MERINGUE. uncomment and run this if you would like. Takes a few hours.
...
# or just load this: 
# com8 = readRDS(paste0('vfish_meringue.rds', '.rds'))
...
```{r, echo=TRUE}
# alt_methods_bank <- readRDS('vfish_0728_giotto_fict_spicemix.rds')
...
png(paste0('spatial_', clustering_name , '.png'), height = 10, width = 10, units = 'in', res = 150)
plotSpatial(bank, 
            by = clustering_name, 
...
dev.off()
knitr::include_graphics(paste0('spatial_', clustering_name , '.png'))
```


Reorder genes, and plot heatmap. 
```{r, echo=TRUE}
# , out.width="10%", fig.cap="VeraFISH heatmap"
...
```{r, echo=TRUE, fig.height=40, fig.width=16}
