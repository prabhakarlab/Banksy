...
multiple spatial omics datasets. We use 10x Visium data of the human 
...
library(ggplot2)
library(ggspavis)
spe <- readRDS(system.file('extdata/Integration.rds', package = 'Banksy'))
...
hvgs <- singleCellTK::getTopHVG(
    singleCellTK::runSeuratFindHVG(
        spe, hvgNumber = 2000, verbose = FALSE), 
    hvgNumber = 2000)
```
...
## Integration in BANKSY's neighborhood-augmented feature space

We run Harmony on the PCs of BANKSY matrix:
...
```{r, eval=T, fig.height=3.5, fig.width=7, fig.align='center'}
um_point_size <- 0.05
um_point_alpha <- 0.05
plot_grid(
    plotReducedDim(spe, 'UMAP_M1_lam0.2', color_by = 'subject_id', 
                   point_size = um_point_size, point_alpha = um_point_alpha) +
        theme(legend.position = 'none'),
    plotReducedDim(spe, 'UMAP_Harmony_BANKSY', color_by = 'subject_id', 
                   point_size = um_point_size, point_alpha = um_point_alpha) + 
...
```{r, eval=T, fig.height=10.5, fig.width=7, fig.align='center'}
sp_point_size <- 0.9
