...
We run BANKSY PCA in lazy mode (the default), which computes the neighborhood
feature matrix and PCA directly without materializing the full BANKSY matrix.
We use `k_geom=18` corresponding to first and second-order neighbors in 10x
Visium.
...
k_geom <- 18
npcs <- 20
spe <- runBanksyPCA(spe, assay_name = aname, lambda = lambda, k_geom = k_geom, npcs = npcs, seed = SEED)
```
...
set.seed(SEED)
harmony_embedding <- harmony::RunHarmony(
    data_mat = reducedDim(spe, "PCA_M0_lam0.2"),
...
spe <- runBanksyUMAP(spe, lambda = lambda, npcs = npcs)
...
plot_grid(
    plotReducedDim(spe, "UMAP_M0_lam0.2",
                   point_size = 0.1,
                   point_alpha = 0.5,
                   color_by = "subject_id") +
