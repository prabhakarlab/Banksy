...
We first create the *Seurat* object by supplying the *Banksy* matrix to *CreateSeuratObject*:

```{r seurat, eval=F}
seu <- CreateSeuratObject(counts = combined$expr)
```

*Seurat* requires that the combined data is scaled. Re-scale the data and run principal component analysis:
...
meta.data(bank)[['res_seurat_clusters']] <- as.numeric(seu@meta.data$seurat_clusters)
plotSpatialDims(bank, params = 'res_seurat_clusters')
```
