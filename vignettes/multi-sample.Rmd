...
package, we first perform basic normalisation of the data, and select the top
2000 highly variable features from each sample. We take the union of these
...
To recapitulate the analysis, the parameters used for analysis are stored in 
the `metadata` slot:

```{r, eval=TRUE, echo=FALSE}
metadata(spe_list$sample_151673)$BANKSY_params
```
