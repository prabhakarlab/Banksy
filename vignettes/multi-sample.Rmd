...
lambda <- 0.2
npcs <- 20
spe_joint <- runBanksyPCA(spe_joint, assay_name = aname, lambda = lambda, k_geom = k_geom, npcs = npcs, group = "sample_id", seed = 1000)
```

Run UMAP on the BANKSY embedding:

```{r, eval=TRUE, message=FALSE, warning=FALSE}
spe_joint <- runBanksyUMAP(spe_joint, lambda = lambda, seed = 1000)
```
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
res <- 0.55
spe_joint <- clusterBanksy(spe_joint, lambda = lambda, resolution = res, seed = 1000)
cnm <- sprintf("clust_M0_lam%s_k50_res%s", lambda, res)
