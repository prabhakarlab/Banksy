...
sample. Here, we demonstrate BANKSY analysis on multiple samples with 4 of the
...
retaining only the counts and some metadata such as the sample identifier and pathology annotations. This saves some memory.
...
Next, select the top 2000 highly variable features for downstream analysis:
...
When running multi-sample BANKSY PCA, the `group` argument *must* be provided. 
This specifies the grouping variable for the cells or spots across the samples.
In this case, `sample_id` in `colData(spe_joint)` gives the grouping based on
...
```{r, eval=T, echo=F}
spe_list <- readRDS(system.file('extdata/DLPFC.rds', package='BanksyBioc'))
M <- 0; lambda <- 0.2; res <- 0.6
