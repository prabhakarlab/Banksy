...
reductions <- rep(c('umap_0.25', 'umap_0.5', 'umap_0.75'), each = 3)
```

Generate UMAPs and Spatial Dimension plots for the above clusters:
```{r out, eval=T}
umaps <- Map(function(by, reduction) {
  plotUMAP(bank, by, reduction, legend = FALSE, pt.size = 0.01, main.size = 10)
  }, clustnames, reductions)


spatdims <- lapply(clustnames, 
                   function(by) plotSpatialDims(bank, by, pt.size = 0.01, main.size = 10))
```
