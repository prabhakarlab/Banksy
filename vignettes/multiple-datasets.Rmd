...
## UMAP
by <- 'res0.8_lam0.25_k40'
umap <- plotUMAP(bank, by, reduction = 'umap_0.25', 
                 pt.size = 0.02, main.size = 10)

## Spatial plots
dsetnames <- names(own.expr(bank))
spatdims <- lapply(dsetnames,
                   function(x) plotSpatialDims(bank, by, dataset = x,
...
*BanksyObject* for multiple datasets can be similarly subset in the one dataset 
case. The main difference is specifying which dataset to subset dimensions by, 
controlled by the `dataset` argument. Here, we subset the *BanksyObject* by 
selecting cells in clusters 1 and 3, while filtering out cells in dataset 2 
by the specified dimensions.

```{r subset, eval=T, fig.width = 8, fig.height = 3.5, fig.align='center'}
bankSubset <- SubsetBanksy(bank, 
                           dims = (sdimx < 5700 | sdimx > 8000) | 
                                  (sdimy < 5000 | sdimy > 8000),
                           dataset = 'd2',
                           metadata = res0.8_lam0.25_k40 %in% c(1,3))

umap <- plotUMAP(bankSubset, by, reduction = 'umap_0.25',
                 pt.size = 0.5, main.size = 10)
spatdim <- plotSpatialDims(bankSubset, by, dataset = 'd2',
                pt.size = 1, main.size = 10)
grid.arrange(umap, spatdim, ncol = 2)
