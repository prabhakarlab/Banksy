...
test_that('Plot heatmap', {
    ...
    expect_s4_class(plotHeatmap(bank, max.cols = 100, assay = 'banksy', 
                                lambda = 0.25), 'Heatmap')
    expect_s4_class(plotHeatmap(bank, assay = 'banksy', dataset = 'd1', 
                                lambda = 0.25, max.cols = 100), 'Heatmap')
})

bank <- RunPCA(bank, lambda = 0.25, npcs = 20)
bank <- RunUMAP(bank, lambda = 0.25, npcs = 20)
...
test_that('Subsetting', {
    ...
    expect_equal(all_cellnames, rownames(bankSubset@reduction$pca_0.25$x))
    expect_equal(all_cellnames, rownames(bankSubset@reduction$umap_0.25))
